        function Get-IDMGroupEntitlement
{
    <#
    .Synopsis
    ...
    begin
    {
        ...
        $uri = "$server/igi/v2/agc/groups/"+ $GroupID +"/entitlement/.search" 
    ...
    process
    {
        ...
        $htmlresponse = Invoke-WebRequest  -uri $uri  -Method post -Headers $headers -SkipCertificateCheck -ContentType "application/scim+json" -Body $body
         ...
         $body='{
            ...
            $FilterString=""
            if( $EntitlementName){
                    $FilterString=$FilterString+' urn:ibm:params:scim:schemas:extension:bean:agc:2.0:Entitlement:name sw \"' + $EntitlementName + '\" and'
            }  
    ...
    {
        Write-Debug "[Get-IDMGroupEntitlement] Complete"
    }
}